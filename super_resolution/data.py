...
class DIV2K:
    def __init__(self,
        ...
        if subset == 'train':
            self.image_ids = glob.glob(f"{images_dir}/DIV2K_train_HR/*.*g")
            
        elif subset == 'valid':
            self.image_ids = glob.glob(f"{images_dir}/DIV2K_valid_HR/*.*g")
        else:
...
#         print(self._hr_cache_index())
    ...
    def _hr_image_files(self):
        ...
        return [image_id for image_id in self.image_ids]

    def _lr_image_files(self):
        images_dir = self._lr_images_dir()
        images = []
        for image_id in self.image_ids:
            path, name = image_id.rsplit("/",1)
            name,ext = name.split(".")
            
            images.append(self._lr_images_dir() + "/" +name + "x4." + ext)
    ...
    def _lr_image_file(self, image_id):
        if not self._ntire_2018 or self.scale == 8:
            return f'{image_id:04}x{self.scale}.jpg'
        else:
            return f'{image_id:04}x{self.scale}{self.downgrade[0]}.jpg'
